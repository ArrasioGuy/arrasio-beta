    ...
    clickables: (() => {
        let Region = (() => {
            ...
            return (size) => {
                ...
                return {
                    place: (index, ...a) => {
                        if (index >= data.length) throw new Error('Out of region!');
                        data[index].set(...a);
                    },
                    hide: () => data.forEach(r => r.hide()),
...
var serverName = 'Prob Not Fixed Server';
window.onload = () => {
    // Server name stuff
    switch (window.location.hostname) {
        case 'localhost:3000': serverName = '🇯🇵 arras-linode-tokyo'; break;
        case 'localhost:3000': serverName = '🇺🇸 arras-linode-newark'; break;
    }
    ...
    function isInView(x, y, r, mid = false) {
        ...
        if (mid) {
            ratio *= 2;
            return x > -global.globalState.screenWidth / ratio - r &&
                x < global.globalState.screenWidth / ratio + r &&
                y > -global.globalState.screenHeight / ratio - r &&
                y < global.globalState.screenHeight / ratio + r;
        }
        return x > -r && x < global.globalState.screenWidth / ratio + r && y > -r && y < global.globalState.screenHeight / ratio + r;
    }
